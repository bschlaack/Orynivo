...
RealtimeAudioPacketizer::RealtimeAudioPacketizer(
    std::uint32_t sampleRate, std::span<const std::uint8_t> audioKey)
    : encoder_(sampleRate), key_(audioKey.begin(), audioKey.end()),
      sequence_(static_cast<std::uint16_t>(readRandom32())), timestamp_(readRandom32()), ssrc_(readRandom32()) {
...
std::vector<std::byte> RealtimeAudioPacketizer::packetize(std::span<const std::byte> pcm) {
    ...
    nonce[0] = static_cast<std::uint8_t>(sequence_);
    nonce[1] = static_cast<std::uint8_t>(sequence_ >> 8);
