...
Hindi uses the appended `Language.Hindi` enum value, the `hi-IN` culture, and
the `hi` website, artist-profile and mobile-remote language code. Do not
renumber persisted enum values. The mobile remote independently selects its
language from the browser locale.

For every new visible string, add the key to Strings.cs and supply its value in
every language initializer. Preserve all composite-format placeholders,
...
languages, website resource completeness and gallery initialization. It does not
replace linguistic review or visual layout testing. The compiled smoke test also
exercises all format strings and switches resources through all languages in
both directions without launching the desktop UI.
