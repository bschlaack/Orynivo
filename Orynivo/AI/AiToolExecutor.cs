...
internal sealed class AiToolExecutor(McpTools tools)
{
    ...
    public Task<string> ExecuteAsync(string name, string argumentsJson, CancellationToken ct = default)
    {
        ...
        return name switch
        {
            ...
            "select_output_profile" => tools.SelectOutputProfileAsync(Str(args, "profile") ?? "", ct),
            "list_equalizer_profiles" => tools.ListEqualizerProfilesAsync(ct),
            "configure_equalizer"   => tools.ConfigureEqualizerAsync(
                                           Str(args, "profile"),
                                           Bool(args, "enabled", false),
                                           ct),
            "get_current_lyrics"    => tools.GetCurrentLyricsAsync(ct),
            "list_orynivo_servers"  => tools.ListOrynivoServersAsync(ct),
            "scan_orynivo_server"   => tools.ScanOrynivoServerAsync(Str(args, "server") ?? "", ct),
