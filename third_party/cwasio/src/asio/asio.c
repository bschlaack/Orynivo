...
#ifdef _WIN32
    // The instance-name selector is a cwASIO extension. Legacy Windows ASIO
    // drivers return inconsistent errors for unknown future() selectors.
    return ASE_OK;
#else
    if(err == ASE_SUCCESS || err == ASE_InvalidParameter)
        return ASE_OK;
